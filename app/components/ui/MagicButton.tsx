...
const MagicButton = ({title,icon,position,handleClick,otherClasses,whatsapp=false}: MagicButtonProps) => {
  return (
    <button className="relative inline-flex h-12 w-full overflow-hidden
     ...
     onClick={handleClick}
     >
      <span className={`absolute inset-[-1000%] animate-[spin_2s_linear_infinite] ${whatsapp ? 'bg-[conic-gradient(from_90deg_at_50%_50%,#dcfce7_0%,#25d366_50%,#dcfce7_100%)]':'bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)]'}`} />
